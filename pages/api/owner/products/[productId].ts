...
async function getHandler(req: NextApiRequest, res: NextApiResponse, token) {
  ...
  const response = await getProductIdAlgolia(productId as string).catch(
    (err) => {
...
async function deleteHandler(req: NextApiRequest, res: NextApiResponse, token) {
  ...
  const response = await deleteByIdProduct(productId as string).catch((err) =>
    res.status(401).send({ message: err })
  );
  res.send(response);
}
...
const patchHandlerWithValidation = schemaBodyAndQuery(
  bodySchemaPatch,
  querySchemaPatch,
  patchHandler
);
